<template>
  <section>
    ...
    <div class="hidden lg:block">
      <h2 class="text-2xl font-bold mb-6">Тестирования</h2>

      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="test in tests" :key="test.id" class="card bg-white shadow-md border border-gray-100 p-6 text-center hover:shadow-xl transition duration-300">
          ...
          <div class="flex-1">
            <div class="space-y-3">
              ...
              <NuxtLink :to="test.testLink" class="btn hover:opacity-90 text-white border-none rounded-full px-8 py-3 w-full shadow-sm"
                :class="savedResults[test.id] ? 'bg-gray-400' : 'bg-sber-red'">
                {{ savedResults[test.id] ? 'Пройти заново' : 'Пройти тест' }}
              </NuxtLink>
            </div>
